...
{
	int row = 0, col = 0;
	cout << "Please enter no. of feilds: ";	cin >> row;
	cout << "\nPlease enter no. of tests conducted for each feild: ";	cin >> col;
	cout << endl;
	if (row <= 0 || col <= 0) {
	...
	if (row > 0 && col > 0)
	{
		...
		do {
			...
			cout << "You choosed: ";	cin >> option;	cout << endl;

	
			switch (option) {
				case 1: {
					int value = 0;
					cout << "Please enter the new value: "; cin >> value;
				...
				case 3: {
					int value = 0;
					cout << "Please enter the value to delete: "; cin >> value;
...
void insert_new(int** arr,int row, int col, int value) {
	int rowNo = 0;
	cout << "Please enter field-no: "; cin >> rowNo; cout << endl;
	if (rowNo > row || rowNo < 0)	cout << "The field-no you entered does'nt exist.\n";
	else
	{
		bool empty = false;
			for (int c = 0; c < col; c++) {
			...
			if (!empty)	cout << "The given field is full\n";
	}
}
...
void max_field(int** arr, int row, int col) {
	...
	cout << "Feild no " << index << " has greatest score of: " << max; cout << endl;
}